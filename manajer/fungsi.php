...
if (isset($_GET['proses2'])) {
	...
	if ($proses2 == 'ubah_jadwal') {
		...
		$input = ("UPDATE jadwal SET id_proyek='$id_proyek', id_sub='$id_sub', tanggal_mulai_j='$tanggal_mulai_j', tanggal_selesai_j='$tanggal_selesai_j', durasi_kegiatan='$durasi_kegiatan' WHERE id_proyek='$id_proyek' AND id_jadwal='$id_jadwal'");
		$query_input = mysql_query($input);
			...
			foreach ($id_pek_pendahulu as $pendahulu) {
				...
				if ($pendahulu !== 'TIDAK') {
					// $input1 = mysql_query("INSERT INTO pendahulu (id_jadwal,id_sub,id_pek_pendahulu) VALUES ('$id_jadwal','$id_sub','$pendahulu')");
										$input1 = mysql_query("UPDATE pendahulu SET id_jadwal='$id_jadwal', id_sub='$id_sub',id_pek_pendahulu='$pendahulu' WHERE id_jadwal='$id_jadwal' and id_sub='$id_sub'");
				}
				//end pendahulu
...
if (isset($_GET['proses_hapus'])) {
	...
	if ($proses_hapus == 'ubah_baca') {
		...
		$page = "progres_pelaksana.php?id_proyek=$id_proyek";
		echo redirectPage($page);
	}
}
?>
...
if (isset($_GET['proses_tambah_detail'])) {
	...
	if ($cek > 0 && tper <= 100) {
		...
		$page = "tambah_progres_pelaksana.php?id_proyek=$id_proyek&id_progres=$id_progres";
		echo redirectPage($page);
	}
